...
/**
 * The seed as a callable, with no side effect on import — `prisma/seed.ts` runs
 * it as a script, and the dev-only reseed route calls it in-process. A module
 * that wiped the database merely by being imported would be a trap.
 */
export const seedDatabase = main;

/** Lets the script runner close the connection this module opened. */
